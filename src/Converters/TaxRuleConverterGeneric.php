...
use \Neomerx\CoreApi\Api\Taxes\TaxRulesInterface as Api;
...
class TaxRuleConverterGeneric implements ConverterInterface
{
    ...
    public function convert($taxRule = null)
    {
        ...
        assert('$taxRule instanceof '.TaxRule::class);

        $result = $taxRule->attributesToArray();
        $result[Api::PARAM_TAX_CODE] = $taxRule->{TaxRule::FIELD_TAX}->{Tax::FIELD_CODE};
...
//    {
//        $tmp = [];
//        foreach ($taxRule->{TaxRule::FIELD_TERRITORIES} as $rule) {
...
//    {
//        $tmp = [];
//        foreach ($taxRule->{TaxRule::FIELD_POSTCODES} as $rule) {
...
//    {
//        $tmp = [];
//        foreach ($taxRule->{TaxRule::FIELD_PRODUCT_TYPES} as $rule) {
...
//    {
//        $tmp = [];
//        foreach ($taxRule->{TaxRule::FIELD_CUSTOMER_TYPES} as $rule) {
